...
const MisCompras = () => {
  ...
  const downloadQR = async (compra) => {
    try {
      ...
      await QRCode.toCanvas(canvas,  `Compra de entrada #${compra.id} para ${compra.movie.name} - Nombre: ${compra.name}, Apellido: ${compra.lastName}, DNI: ${compra.dni}`, { width: 256 });
