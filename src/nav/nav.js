...
const Nav = () => {
  ...
  const {  setCurrentMovieId, currentMovieId, setCurrentTickets } = useSalesContext();
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Error al obtener los movies");
        }
      ...
      <div className="container">
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          ...
          <li className="nav-item">
            ...
            <li className="nav-item">
              <Link className="nav-link text-white" to="/movies">Peliculas</Link>
            </li>
            {token && (
              <li className="nav-item">
            ...
            {token && currentMovieId > 0 && (
              <li className="nav-item">
                <Link className="nav-link text-white" to={`/comprar/${currentMovieId}`}>Continuar compra</Link>
              </li>
            )}
            {token ? (
              <li className="nav-item">
